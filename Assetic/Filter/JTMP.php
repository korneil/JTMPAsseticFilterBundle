    ...
    private function compileJTmp($jData,$breakLines=false){
        ...
        for($i=0;$i<$cB;$i++){
            ...
            for($j=0;$j<$cM;$j++){
                ...
                if(strlen($before)) $res.='r+="'.addslashes($before).'";'.$lb;
                switch($m[1][$j][0]){
                    case '$':
                        $h=explode('|',$m[2][$j][0]);
                    ...
                    case '@':
                        if($m[2][$j][0]=='') $res.='}'.$lb;
                            ...
                            if(isset($h[2])){
                                $res.='for('.$h[1].' in '.$h[0].'){'.$this->createJTmpVar($h[2]).'='.$h[0].'['.$h[1].'];'.$lb;
                            }else{
                    ...
                    case '?':
                        if($m[2][$j][0]=='') $res.='}'.$lb;
                        else $res.='if('.$this->createJTmpVar($m[2][$j][0]).'){'.$lb;
                        break;
                    case '!':
                        $res.='}else{'.$lb;
                        break;
            ...
            if(strlen($h)) $res.='r+="'.addslashes($h).'";'.$lb;
            $res.='return r;}'.$lb;
            if($i<$cB-1) $res.=','.$lb;
        }
        $res.='}'.$lb;
